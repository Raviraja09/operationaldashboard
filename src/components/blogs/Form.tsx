...
const Form = ({ title, setData, data, setCancel }: FormProps) => {
  ...
  const blog_id = useRef<HTMLInputElement>(null);
  ...
  const handleClick = async (e: any) => {
    e.preventDefault();
    const blogids = blog_id.current?.value;
    const tittles = tittle.current?.value;
...
debugger;
      ...
      const response = await fetch(
        "https://diet-ideas-production.up.railway.app/v1/blog",
        {
          method: "POST",
    ...
    if (title?.includes("Edit")) {
      ...
      const dataid = blogids;
      alert(dataid);
      try {
       
      const response = await fetch(
        `https://diet-ideas-production.up.railway.app/v1/blog/${dataid}`,
        {
          method: "PUT",
          body: JSON.stringify(data),
          headers: {
  ...
  useEffect(() => {
    if (title?.includes("Edit")) {
      blog_id.current!.value = data?.blog_id;
      tittle.current!.value = data?.title;
